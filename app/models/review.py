...
from sqlalchemy import Boolean, DateTime, Integer, Text, ForeignKey, CheckConstraint
...
class TripReview(Base):
    ...
    __table_args__ = (
        ...
        CheckConstraint("bus_condition_rating IS NULL OR (bus_condition_rating >= 1 AND bus_condition_rating <= 5)", name="ck_bus_condition_rating"),
        CheckConstraint("punctuality_rating IS NULL OR (punctuality_rating >= 1 AND punctuality_rating <= 5)", name="ck_punctuality_rating"),
        CheckConstraint("comfort_rating IS NULL OR (comfort_rating >= 1 AND comfort_rating <= 5)", name="ck_comfort_rating"),
    )

    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    booking_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("bookings.id"), unique=True, nullable=False, index=True)
    ...
    driver_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("drivers.id"), index=True)
    ...
    admin_responded_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    admin_responded_by: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"))
